...
{
	numTurnMoves = numTurnMove;
	for (int i = 0; i<numTurnMoves; i++) // get the values while tele-op is running
	{
		while(joy1Btn(1) != 1){}
		encoder[i] = nMotorEncoder[motorFL];
	}
...
task autonomousFile()
{
  ...
  for ( int i=1; i < 15; i++){
	...
	ReadShort(fileHandle, ioResult, nParm);
	Close(fileHandle, ioResult);
}
...
//===================== Initizlaize Driver Controls (In Main Task) =====================
	...
	// Initialize the Autonomous File Recorder Method.
	StartTask(autonomousFile);
